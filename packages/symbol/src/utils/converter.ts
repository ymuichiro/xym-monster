...
const SIGNEDNESS_AND_SIZE_TO_ARRAY_TYPE_MAPPING: any = {
  false: {
    1: Uint8Array,
    2: Uint16Array,
    4: Uint32Array,
 ...
 */
export const toByte = (char1: string, char2: string): number => {
 ...
 */
export const hexToUint8 = (input: string): Uint8Array => {
  if (0 !== input.length % 2) throw Error(`hex string has unexpected size '${input.length}'`);

  const output = new Uint8Array(input.length / 2);
  for (let i = 0; i < input.length; i += 2) output[i / 2] = toByte(input[i], input[i + 1]);

  return output;
};

export const utf8ToHex = (utf8String: string) => {
  const encoder = new TextEncoder();
  const utf8Bytes = encoder.encode(utf8String);

  const hexArray = Array.from(utf8Bytes, (byte) => byte.toString(16).padStart(2, '0'));
 ...
 */
export const uint8ToHex = (input: Uint8Array): string => {
  let s = '';
  input.forEach((byte) => {
 ...
 */
export const bytesToInt = (input: Uint8Array, size: number, isSigned: any = false): number | BigInt => {
 ...
 * @returns {numeric|BigInt} Value corresponding to the input.
 */
export const bytesToIntUnaligned = (input: Uint8Array, size: number, isSigned: any = false) => {
 ...
 */
export const intToBytes = (value: number, byteSize: number, isSigned: any = false): Uint8Array => {
